...
from app.auto_context.chunker import chunk_latex, hierarchy_path
...
async def index_project(project_id: str, user_id: str) -> None:
    ...
    async with get_async_session() as db:
        ...
        try:
            ...
            tasks = [
                _index_file_tracked(project_id, f, db, sem)
                for f in indexable
            ]
            await asyncio.gather(*tasks, return_exceptions=True)
            # return_exceptions=True: _index_file_tracked never raises;
            # errors are already written to rag_file_index
            ...
            # Only hits if get_project_files itself fails
