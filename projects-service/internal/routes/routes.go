...
func SetupRoutes(r *gin.Engine, h *handlers.Handler, authMiddleware *middleware.AuthMiddleware) {
	...
	// Collaborators
	api.GET("/projects/:projectID/access", h.GetAccess)
	...
	api.GET("/projects/:projectID/collaborators/links", h.ListInviteLinks)
	api.DELETE("/projects/:projectID/collaborators/links/:inviteID", h.RevokeInviteLink)
	...
	internal.GET("/project/:projectID/download", h.InternalDownloadProject)
}
